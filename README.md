...
- TypeScript
  ...
  - locationScript.ts
    ...
    - OpenRoute API
      - Additionally, successCallback() calls on getDistanceData(), which is a function that takes typed parameters (latitude and longitude of both locations), and calls on fetchDistanceData(). fetchDistanceData() takes in typed parameters and returns a DistanceData type (which is an interface defined in the script), and tries the API call to get the distance data. In the else condition, a safe fallback of an empty array is provided to avoid undefined. getDistanceData() takes the return DistanceData type, and if data and routes are not null/undefined, and length is greater than 0, then distance data is summarized into text content for the distanceData element. If the returned array doesn't meet that criteria, then a descriptive message is returned.
    - Geoapify Places API
      - This section runs when the page is rendered. getLocalData() is called in Location.jsx. getLocalData() calls on fetchLocalData(), which returns a GeoapifyResponse (an interface defined in the code). It calls the Geoapify Places API with my key and looks for restaurants within one mile of the venue coordinates, limiting to 20 results. In the else condition, a safe fallback of an empty array is provided to avoid undefined. getLocalData() takes the returned GeoapifyResponse type,and if data and features are not null/undefined, and length is greater than 0, then the data is mapped to the Place class which is defined in the code. The Place class uses the PlaceProperties interface to define expected properties, splits the cuisine string at the semi-colons if there are multiple cuisines listed for the Place, and replaces underscores in the cuisine string with spaces. The places are then appended using the Place class function toCardElement(), which creates an article element, assigns the localCard class, assigns the cuisine tags, and produces HTML for that card.
